...
def _run_one_experiment(task):
    """Worker: runs a single seeded (problem, config) experiment.

    Runs in its own process under --workers > 1, so seeding the
    process-global RNGs is safe (each worker has independent RNG state).
    Seeds are derived per experiment, so parallel results are IDENTICAL to
    sequential results.
    """
    factory, config, num_runs, max_iterations = task
...
def run_campaign(problem_factories, configs, num_runs, max_iterations,
    ...
    With workers > 1, experiments run in parallel processes. Output files
    never conflict (one CSV pair per experiment), and per-experiment
    seeding guarantees results identical to a sequential run.
    """
    tasks = [
        (factory, config, num_runs, max_iterations)
...
def main():
    ...
    parser.add_argument("--workers", type=int, default=1,
                        help="parallel experiment processes (each experiment "
    ...
    if not args.aggregate_only:
        if args.suite in ("all", "unconstrained"):
            run_campaign(
                [SCH1, ZDT1, ZDT2, ZDT3, ZDT4, ZDT6],
                [mgpso_config("MGPSO")],
                num_runs, max_iterations, workers=args.workers,
            )
